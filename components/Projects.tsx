...
export default function Projects({ className = '' }: ProjectsProps) {
  ...
  const renderProject = ({ title, description, tech, github, featured }: typeof projects[0], index: number) => (
    ...
    >
      <div className={`group relative backdrop-blur-sm border rounded-2xl p-6 shadow-elegant hover:shadow-glow transition-all duration-500 hover:scale-[1.02] overflow-hidden flex flex-col h-full ${
        featured ? 'border-primary/40 bg-primary/5' : 'bg-card/30 border-border hover:border-primary/40'
      }`}>
            ...
            {tech.slice(0, 4).map((t) => (
              <span
                ...
                className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors duration-300 ${
                  featured 
                    ? 'bg-primary/10 text-primary border-primary/30 group-hover:border-primary/50'
                    : 'bg-secondary/80 text-secondary-foreground border-border/50 group-hover:border-primary/30'
                }`}
              >
                {t}
              </span>
        ...
        <ScrollAnimation animation="fade-in-up" delay={200}>
          <div className="mb-20 flex flex-col items-center gap-8">
            {featuredProjects.map((project, index) => (
              <div key={project.title} className="w-full max-w-3xl">
